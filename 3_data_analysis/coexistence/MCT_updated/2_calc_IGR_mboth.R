...
source("3_data_analysis/coexistence/MCT_updated/1_calc_equilibrium_mboth.R")
...
# Create Functions ####
## static igr
igr_stat_func = function(surv, germ, lambda, alpha_intra, Nt, alpha_inter, 
                         germ_inter, inter_abund) {
  
  Ntp1 = (1-germ)*surv*Nt + germ*lambda*Nt*exp(alpha_intra *germ* Nt + 
                                                 alpha_inter*germ_inter*inter_abund)
...
## m0 ####
## create empty df
igr_m0 = data.frame(focal = NA, water = NA, post_num = NA, igr = NA, 
                    equil = NA, alpha_inter = NA, lambda = NA)
