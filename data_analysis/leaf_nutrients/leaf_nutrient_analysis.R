...
## dens x water x microbe patterns
CN_final %>%
  group_by(ACAM, water, microbe) %>%
  summarise(mean_d13 = mean(delta13C),
            se_d13 = calcSE(delta13C)) %>%
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
# ggsave("figures/Apr2025/leaf_nutrient_story/alldens_d13C.png", width = 8, height = 3.5)

## dens x water x microbe patterns, lo/high water only
CN_final %>%
  filter(water != 0.75) %>%
  group_by(ACAM, water, microbe) %>%
  summarise(mean_d13 = mean(delta13C),
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
ggsave("figures/Apr2025/leaf_nutrient_story/water_lo_hi_d13C.png", width = 8, height = 3.5)
...
## dens x water patterns x microbe
CN_final %>%
  group_by(ACAM, water, microbe) %>%
 # filter(microbe == 1) %>%
  summarise(mean_d15 = mean(delta15N),
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
# ggsave("figures/Apr2025/leaf_nutrient_story/alldens_d15N.png", width = 8, height = 3.5)
...
CN_final %>%
 ...
 # filter(water != 0.75) %>%
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
ggsave("figures/Apr2025/leaf_nutrient_story/alldens_d15N_faceted.png", width = 10, height = 3.5)
...
CN_final %>%
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
#ggsave("figures/Apr2025/leaf_nutrient_story/alldens_%N_faceted.png", width = 10, height = 3.5)

## water x dens in live soil only
CN_final %>%
  filter(microbe == 1) %>%
  group_by(ACAM, water, microbe) %>%
  summarise(mean_N = mean(WtN),
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
#ggsave("figures/Apr2025/leaf_nutrient_story/alldens_livesoil_%N.png", width = 8, height = 3.5)

## water x dens x microbe in lo / high water only
CN_final %>%
  filter(water != 0.75) %>%
  group_by(ACAM, water, microbe) %>%
  summarise(mean_N = mean(WtN),
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
#ggsave("figures/Apr2025/leaf_nutrient_story/water_lo_hi_%N.png", width = 8, height = 3.5)
...
CN_final %>%
  ...
  mutate(ACAM = as.numeric(ACAM),
         water.text = ifelse(water == 1, "High",
                             ifelse(water == 0.75, "Intermediate", "Low")),
         microbe.text = ifelse(microbe == 1, "Live", "Sterilized")) %>%
  ggplot(aes(x=ACAM, y=mean_CN, shape = as.factor(microbe.text), fill = water.text, group = interaction(water, microbe))) +
  geom_errorbar(aes(ymin = mean_CN - se_CN, ymax = mean_CN + se_CN), width = 2) +
  geom_line() +
  geom_point(size = 3) +
  xlab("Legume Density") +
  ylab("CN Ratio") +
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
#ggsave("figures/Apr2025/leaf_nutrient_story/alldens_CN_faceted.png", width = 10, height = 3.5)
...
CN_final %>%
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
#ggsave("figures/Apr2025/leaf_nutrient_story/alldens_livesoil_CN.png", width = 8, height = 3.5)
...
CN_final %>%
  ...
  scale_shape_manual(values = c(21, 22)) +
  labs(shape = "Soil Treatment")
#ggsave("figures/Apr2025/leaf_nutrient_story/water_lo_hi_CN.png", width = 8, height = 3.5)
