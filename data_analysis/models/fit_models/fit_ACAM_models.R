...
for(i in rainfall){
    ...
    ## set initial values 
    initials1 <- list(lambda=10, N_opt = 1, c = 0, alpha_slope = -0.2, alpha_initial = -0.15, alpha_acam = 0.1)
    
    initials2 <- list(lambda=25, N_opt = 0, c = -0.5, alpha_slope = -0.5, alpha_initial = -0.2, alpha_acam = -0.1)
    
    initials3 <- list(lambda=75, N_opt = 5, c = -0.1, alpha_slope = -0.4, alpha_initial = -0.1, alpha_acam = -0.4)
    
    initials4 <- list(lambda=40, N_opt = 2, c = -0.3, alpha_slope = 0, alpha_initial = 0.3, alpha_acam = -0.2)
    ...
    ## run the model
    sigmoidal.output[[paste0("acam_w", i)]] = stan(file = 'data_analysis/models/fit_models/ACAM_ricker_nb_sigmoidal.stan', data = data_vec, #init = initialsall,
                                                   iter = 18000, chains = 4, thin = 2, control = list(adapt_delta = 0.99, max_treedepth = 18))
