...
transformed parameters{
  ...
  for(i in 1:N){
    ...
    // what if rather than fitting a dev term for every part of the alpha function, what if I set an overall dev??
    // not sure how this would work, leave for now.

    F_hat[i] = (lambda + lambda_dev*trt[i]) * exp( (acam[i]*(alpha_acam[i])) + (N_i[i]*(alpha_brho + alpha_brho_dev*trt[i]))) ;
    
    //+ alpha_brho_dev*trt[i] taking out this param, as it is estimating a relatively wide variance, but we don't have data for this parameter
  }
...
model{
  ...
  alpha_initial ~ normal(0.099, 0.15);
  alpha_initial_dev ~ normal(0, 0.05); // change variance from 0.25 -> 0.05
  
  //alpha_slope ~ normal(0, 0.1);
  alpha_slope ~ normal(-0.2, 0.15); // try this out just to see what happens?
  // want value somewhat near 0; aiming to make the prior a straight line
  alpha_slope_dev ~ normal(0, 0.05);

  c ~ normal(-0.1, 0.15);
  c_dev ~ normal(0, 0.05);
  
  N_opt ~ exponential(0.7); 
  // N_opt = the optimal density of ACAM that maximizes fecundity of ACAM
  // try giving this one a wider range to see what it does? 
  
  N_opt_dev ~ normal(0, 0.05);
