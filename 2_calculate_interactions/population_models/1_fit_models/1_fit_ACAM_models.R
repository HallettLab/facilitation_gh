...
stat.out = list() ## make a list for model output
...
for(i in rainfall){
    ...
    ## run the model
    stat.out[[paste0("acam_w", i)]] = stan(file = 'data_analysis/models/fit_models/ACAM_ricker_nb_static.stan', 
                                               data = data_vec, init = initialsall, 
                                               iter = 5000, chains = 4, thin = 2, 
                                               control = list(adapt_delta = 0.9,
                                                              max_treedepth = 18))
    
    PrelimFit = stat.out[[paste0("acam_w", i)]]
    
    ## save model output
    save(PrelimFit, file = paste0("data_analysis/models/output/static/", date, 
                                  "/acam_nb_static_w", i, "_", date, ".rdata"))
...
for(i in rainfall){
    ...
    ## run the model
    sig.out[[paste0("acam_w", i)]] = stan(file = 'data_analysis/models/fit_models/ACAM_ricker_nb_sigmoidal.stan', 
                                                   data = data_vec, init = initialsall, 
                                                   iter = 5000, chains = 4, thin = 2, 
                                                   control = list(adapt_delta = 0.999, 
                                                                  max_treedepth = 18))
    
    PrelimFit = sig.out[[paste0("acam_w", i)]]
    
    ## save model output
    save(PrelimFit, file = paste0("data_analysis/models/output/sigmoidal/", 
                                  date, "/acam_nb_sigmoidal_w", i, "_", date, 
                                  "_final.rdata"))
